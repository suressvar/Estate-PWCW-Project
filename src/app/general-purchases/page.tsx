...
import { GeneralPurchaseLogItem } from "@/lib/transaction-logs";
import { DollarSign, ShieldAlert, SlidersHorizontal } from "lucide-react";
import { RoleBadge } from "@/components/ui/role-badge";

export default function GeneralPurchasesPage() {
  const [logs, setLogs] = useState<GeneralPurchaseLogItem[]>([]);
  const [canEdit, setCanEdit] = useState(true);
  const [roleName, setRoleName] = useState("Admin");

  const fetchLogs = async () => {
    const res = await fetch("/api/general-purchase-logs");
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {!canEdit ? (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-md text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>Access Restricted: You do not have permission to log General Purchase entries for this module.</span>
        </div>
      ) : (
        <CascadeLogForm
          moduleTitle="General Purchase Log"
          submitEndpoint="/api/general-purchase-logs"
          onSuccess={fetchLogs}
          initialExtraState={{ description: "Drip Irrigation Pipe Fittings & Valves", cost: "8500" }}
          renderExtraFields={(state, setState) => (
            <>
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-700">Total Purchase Cost (₹)</label>
                <input
                  id="gen-cost-input"
                  type="number"
                  required
          ...
          <tbody className="divide-y divide-slate-200/80 bg-white">
            {logs.map((log) => (
              <tr key={log.id} className="hover:bg-slate-50/80">
                ...
                <td className="p-3 font-semibold text-slate-800">{log.description}</td>
                <td className="p-3 font-bold text-red-600">₹{log.cost}</td>
