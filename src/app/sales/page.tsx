...
import { SalesLogItem } from "@/lib/transaction-logs";
import { TrendingUp, ShieldAlert, SlidersHorizontal } from "lucide-react";
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-5 rounded-lg border border-slate-200 shadow-xs gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-bold text-slate-900">Crop Sales & Revenue Logs</h1>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Log crop sales, quantities sold, revenue earned, and buyer details.
          </p>
        </div>
      ...
      {!canEdit ? (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-md text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>Access Restricted: You do not have permission to log Sales entries for this module.</span>
        </div>
      ) : (
        <CascadeLogForm
          moduleTitle="Sales Entry"
          submitEndpoint="/api/sales-logs"
          onSuccess={fetchLogs}
          initialExtraState={{ quantityKg: "1000", value: "45000", buyerName: "Koyambedu Traders" }}
          renderExtraFields={(state, setState) => (
            <>
              <div className="space-y-1">
                <label className="font-semibold text-slate-700">Quantity Sold (kg)</label>
                <input
                  id="sales-qty-input"
                  type="number"
                  required
              ...
              <div className="space-y-1">
                <label className="font-semibold text-slate-700">Revenue Received (₹)</label>
                <input
                  id="sales-val-input"
                  type="number"
                  required
              ...
              <div className="space-y-1 sm:col-span-2">
                <label className="font-semibold text-slate-700">Buyer Name / Merchant</label>
                <input
      ...
      <div className="bg-white border border-slate-200 rounded-lg shadow-xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 font-bold text-xs text-slate-900">
          Sales & Revenue History ({logs.length})
        </div>
          ...
          <tbody className="divide-y divide-slate-200/80 bg-white">
            {logs.map((log) => (
              <tr key={log.id} className="hover:bg-slate-50/80">
                ...
                <td className="p-3 font-semibold text-slate-800">{log.quantityKg} kg</td>
                <td className="p-3 font-bold text-emerald-700">₹{log.value}</td>
                <td className="p-3 text-slate-600">{log.buyerName || "—"}</td>
